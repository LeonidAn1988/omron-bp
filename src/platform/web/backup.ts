 ...
 *
 * Есть это не везде: Chrome и Edge на компьютере — да, Safari и мобильные
 * браузеры — нет. Там `isSupported()` вернёт false, и приложение предложит
 * сохранять копию вручную.
 */
...
export const webBackup: BackupPort = {
  ...
  async choose(suggestedName) {
    ...
    try {
      ...
      // Отказ от диалога — не ошибка, а решение пользователя.
