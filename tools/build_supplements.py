...
# Служебные разделы в конце области применения. Читать их незачем, а вреда от
# них хватает: «хранить вдали от источников тепла» давало добавке источник
# «тепла» — почти сотня записей.
SERVICE = re.compile(
    r'(Рекомендаци\w+ по применению|Условия хранения|Срок годности|Противопоказани\w+|'
    r'Не являетс\w+ лекарств\w+)',
    re.I,
)
...
def total_pages(body: str) -> int:
    ...
    if not found:
        # Молчаливая единица здесь стоила бы справочника: сборка прошла бы за
        # секунду, положила бы сто записей вместо двадцати пяти тысяч, и
        # заметить это было бы негде — файл уже в поставке.
        raise SystemExit('счётчик страниц не найден: вёрстка реестра изменилась')
    return int(found.group(1))


def actual_date(body: str) -> str | None:
    """Дата актуализации реестра — со страницы, а не со стенных часов.

    Раньше здесь стояла `date.today()` с пояснением, что реестр даты не
    сообщает. Он сообщает: «Дата актуализации базы данных&nbsp;&#151;
    <b>08.09.2026 г.</b>». Разница не косметическая по двум причинам.
    Во-первых, дату видит человек в приложении, и она должна говорить о
    данных, а не о том, когда запускали скрипт. Во-вторых, страницы лежат в
    вечном кэше `tools/out/sgr`: повторный прогон занимает секунды и прежде
    ставил сегодняшнее число на августовские данные — то есть врал тем
    убедительнее, чем чаще пересобирали.
    """
    found = re.search(r'Дата актуализации базы данных.{0,40}?(\d{2})\.(\d{2})\.(\d{4})', body, re.S)
    if not found:
        return None
    d, m, y = found.groups()
    return f'{y}-{m}-{d}'
...
def main() -> None:
    ...
    data = build(records)
    # Дата — со страницы реестра. Своей датой подменяем только когда реестр
    # промолчал, и тогда об этом говорим вслух: тихая подмена и была дефектом.
    заявленная = actual_date(first)
    if заявленная is None:
        заявленная = date.today().isoformat()
        print('реестр не назвал дату актуализации — ставим дату сборки', file=sys.stderr)
    data['date'] = заявленная
