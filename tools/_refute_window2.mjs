/** Чистое окно внутри страницы: от самого события click до ухода кнопки из DOM. */
...
async function измерить(throttle, шт) {
  ...
  // Наблюдатель ставится ДО клика, всё внутри страницы — без накладных CDP.
  await page.evaluate(([x,y]) => {
    const кнопка = document.elementFromPoint(x,y)
    window.__t = null; window.__окно = new Promise((res) => {
      document.addEventListener('click', () => { window.__t = performance.now() }, true)
      const mo = new MutationObserver(() => {
        if (window.__t !== null && !document.contains(кнопка)) { mo.disconnect(); res(Math.round(performance.now() - window.__t)) }
      })
      mo.observe(document.body, { childList: true, subtree: true })
    })
  }, [x,y])
  await page.mouse.click(x, y)
  const окно = await page.evaluate(() => window.__окно)
  await ctx.close()
  return окно
}
...
for (const шт of [1, 12]) for (const t of [1, 8, 20]) {
  const пробы = []
  for (let i = 0; i < 3; i++) пробы.push(await измерить(t, шт))
  console.log(`аптечка ${String(шт).padStart(2)} преп.  CPU x${String(t).padStart(2)}  чистое окно, мс: ${пробы.join(', ')}`)
}
await browser.close()
