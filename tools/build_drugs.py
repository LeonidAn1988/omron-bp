...
# Из разбора форм их убираем, но не выбрасываем: по ним считается признак `r`.
RX_NOISE = re.compile(r'-\s*(Без рецепта|По рецепту)\s*;?', re.I)

# Тот же текст, но чтобы прочитать, а не стереть. В сегодняшней выгрузке
# пометка стоит у 97,3 % строк: «по рецепту» 62,2 %, «без рецепта» 34,6 %.
RX_MARK = re.compile(r'-\s*По рецепту', re.I)
...
def build(blob: bytes) -> dict:
    ...
    for row in sheet.iter_rows(min_row=FIRST_DATA_ROW, values_only=True):
        ...
        item = merged.setdefault(
            trade.lower(), {'n': trade, 'i': mnn, 'makers': set(), 'forms': {}, 'homeo': False, 'rx': False}
        )
        if not item['i'] and mnn:
            item['i'] = mnn
        if is_homeopathic(raw_forms, raw_group):
            item['homeo'] = True
        # Достаточно одной рецептурной формы. У 73 наименований формы разной
        # отпускной категории — «Омепразол Велфарм» и подобные; человек держит
        # в руках одну пачку, и предупредить его о походе к врачу дешевле, чем
        # промолчать. Признак в приложении правится руками.
        if RX_MARK.search(raw_forms):
            item['rx'] = True
    ...
    for item in merged.values():
        ...
        if item['i']:
            record['i'] = item['i']
        # Отпускается по рецепту. Пометка приезжает в том же поле, что и форма
        # выпуска, и до сих пор стиралась вместе с прочим шумом. Она меняет
        # срок, за который надо спохватиться: за неделю к врачу не успеть.
        if item['rx']:
            record['r'] = 1
        if variants:
            record['v'] = variants
