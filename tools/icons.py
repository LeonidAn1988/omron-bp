...
# Значок уведомления кладётся прямо в ресурсы Android, минуя `@capacitor/assets`:
# тот про уведомления ничего не знает и раскладывает только значки приложения.
NOTIF_OUT = Path('android/app/src/main/res')

# Плотности экрана и сторона значка уведомления в пикселях. Android требует 24dp;
# в пикселях это 24 на mdpi и дальше по множителю плотности.
NOTIF_SIZES = {
    'drawable-mdpi': 24,
    'drawable-hdpi': 36,
    'drawable-xhdpi': 48,
...
def main() -> None:
    ...
    # Значок уведомления. Строка состояния красит его сама и берёт **только
    # альфа-канал**: цветную картинку она превратит в белый прямоугольник, и
    # вместо кардиограммы человек увидит квадрат. Поэтому рисунок белый на
    # прозрачном, без фона и без обводки.
    #
    # Занимает не весь квадрат: система оставляет вокруг значка воздух, и
    # рисунок во всю ширину выглядит крупнее соседних системных значков.
    for каталог, сторона in NOTIF_SIZES.items():
        путь = NOTIF_OUT / каталог
        путь.mkdir(parents=True, exist_ok=True)
        draw_mark(сторона, 0.92).save(путь / 'ic_stat_omron.png')
    print(f'значок уведомления: {len(NOTIF_SIZES)} плотностей')
