...
for (const width of [407, 380, 360, 340, 330, 320]) {
  ...
  await page.evaluate(() => { document.documentElement.setAttribute('data-theme','dark'); document.documentElement.setAttribute('data-text','xlarge') })
  ...
  console.log(`docW=${r.docW}  Принято ${f(r.prinyato)}   100% ${f(r.hundred)}   Дозировка ${f(r.dozirovka)}`)
  await browser.close()
}
